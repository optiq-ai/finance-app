-- Schemat bazy danych PostgreSQL dla aplikacji analizy finansowej
...
-- Zastosowanie triggera dla wszystkich tabel
...
-- Funkcja do weryfikacji duplikatów wypłat
CREATE OR REPLACE FUNCTION check_payroll_duplicate(
...
-- Funkcja do weryfikacji duplikatów sprzedaży
CREATE OR REPLACE FUNCTION check_sale_duplicate(
    p_invoice_number VARCHAR,
...
('Utrzymanie', 'UTRZ', 'Usługi utrzymaniowe'),
('Zarządzanie', 'ZZ', 'Usługi zarządzania'),
('Kara', 'KARA', 'Kary umowne'),
('Profesjonalne', 'PROFI', 'Usługi profesjonalne'),
('Handel', 'HANDEL', 'Usługi handlowe');
...
('Telekomunikacyjne i informatyczne', 'Koszty usług telekomunikacyjnych i informatycznych'),
('Paliwo', 'Koszty paliwa'),
('Usługi pozostałe', 'Pozostałe koszty usług');

-- Podstawowe oddziały
INSERT INTO departments (name, code, description)
VALUES 
('ZAMOŚĆ', 'ZAM', 'Oddział w Zamościu'),
('KALISZ', 'KAL', 'Oddział w Kaliszu'),
('LEGNICA', 'LEG', 'Oddział w Legnicy'),
('ROMAN', 'ROM', 'Oddział Roman'),
('KAMIL', 'KAM', 'Oddział Kamil'),
('ADMINISTRACJA', 'ADM', 'Dział administracji'),
('NEXO', 'NEX', 'Oddział Nexo'),
('ZARZĄD', 'ZAR', 'Zarząd firmy'),
('HANDEL', 'HAN', 'Dział handlowy'),
('PROJEKTY', 'PRO', 'Dział projektów');

-- Podstawowe grupy
INSERT INTO groups (name, department_id, description)
VALUES 
('BIURO', (SELECT id FROM departments WHERE name = 'ZAMOŚĆ'), 'Biuro oddziału Zamość'),
('DOST', (SELECT id FROM departments WHERE name = 'ZAMOŚĆ'), 'Dostawy oddziału Zamość'),
('BIERNACICE', (SELECT id FROM departments WHERE name = 'KAMIL'), 'Grupa Biernacice'),
('UTRZ', (SELECT id FROM departments WHERE name = 'KALISZ'), 'Utrzymanie oddziału Kalisz'),
('JAKUB', (SELECT id FROM departments WHERE name = 'ROMAN'), 'Grupa Jakub'),
('USŁUGI', (SELECT id FROM departments WHERE name = 'ADMINISTRACJA'), 'Usługi administracyjne');

-- Indeksy analityczne

-- Indeks dla analizy kosztów w czasie
CREATE INDEX idx_purchases_analysis_time 
ON purchases(date, department_id, service_type_id, cost_category_id);

-- Indeks dla analizy wypłat w czasie
CREATE INDEX idx_payroll_analysis_time
ON payroll(year, month, department_id, group_id, service_type_id);

-- Indeks dla analizy sprzedaży w czasie
CREATE INDEX idx_sales_analysis_time
ON sales(date, department_id, service_type_id);
